...
export default async function handler(
  ...
  try {
    ...
    let query = supabase.from("candidats").select("*", { count: "exact" }); // Ajout du count
    ...
    if (type_exp) {
      switch (type_exp) {
        ...
        case "intermediaire":
          query = query
            .gte("annees_experience", 5)
            .lte("annees_experience", 15);
          break;
        case "senior":
    ...
    console.log(
      `✅ ${data?.length || 0} candidats récupérés (total: ${count})`,
    );
    ...
    if ((min_exp || max_exp || type_exp) && (!data || data.length === 0)) {
      console.log(
        "⚠️ Filtre par expérience mais colonne annees_experience peut être vide",
      );
    }
